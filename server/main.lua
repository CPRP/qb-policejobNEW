...
RegisterNetEvent('police:server:TakeOutImpound', function(plate, garage)
    ...
    if #(playerCoords - targetCoords) > 10.0 then return end
		-- Removed DropPlayer(src, "Attempted exploit abuse") from before "then" above
...
QBCore.Functions.CreateCallback('police:server:GetPlayerWarrants', function(source, cb)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local cid = Player.PlayerData.citizenid
    local name = Player.PlayerData.charinfo.firstname..' '..Player.PlayerData.charinfo.lastname 
